...
class PerceiverLayer(nn.Module):
    def __init__(self,max_freq, n_head, d_head, d_byte_arr, d_latent, d_kv, input_type, device , 
                 num_bands = 4, cross_atten_dropout = 0.1,self_atten_dropout = 0.1, 
                 cross_ff_dropout =0.1, latent_ff_dropout = 0.1):
        super().__init__()
        self.cross_atten = Attention(n_head, d_byte_arr, d_latent, d_kv, d_kv, temp=d_head ** -0.5, dropout=cross_atten_dropout)
        self.self_atten = LatentTransformer(n_head, d_head, d_kv, d_latent, self_atten_dropout)
        self.positional_encoding = FourierFeaturePositionEncoding(max_freq, d_byte_arr, input_type, device, num_bands)
        self.byte_FFN = PositionWiseFeedforward(d_latent,dropout=cross_ff_dropout)
        self.latent_FFN = PositionWiseFeedforward(d_latent,dropout=latent_ff_dropout)
